...
.  **Get the console route URL**
    You can filter routes by the `ActiveMQArtemis=amq-broker` label, or specifically search for routes matching the console naming pattern:

    ```bash
    oc get routes -l ActiveMQArtemis=amq-broker
    ```
