...
oc get pods -l ActiveMQArtemis=amq-broker-secure-example
----
...
[source,bash]
----
oc logs $(oc get pod -l ActiveMQArtemis=amq-broker-secure-example -o jsonpath='{.items[0].metadata.name}') -n $(oc project -q) | grep "acceptor"
----
