...
*   **Custom Resource (CR):** A CR is an actual instance of an object defined by a CRD. When you want to deploy an AMQ Broker, you create a `Broker` CR (e.g., `kind: ActiveMQArtemis`), specifying its desired state (number of replicas, persistence settings, security configurations, etc.).
...
. Login to https://access.redhat.com/[Red Hat Customer Portal, window=_blank] using your credentials.
. Click **Downloads** option on top of the page.
. Look for **Red Hat AMQ Broker** option on the page and click on it.
. On the **Download AMQ Broker** page, look for **Operator Installation and Example Files**
. Click on the available option.
. On the next page, click **Download Release** button to download the **amq-broker-operator-x.y.z-ocp-install-examples-rhel9.zip** file.
+
NOTE: The version number x.y.z will vary.
. Copy downloaded file to your lab environment.
+
.Example command
+
----
scp -P 31233 /tmp/amq-broker-operator-7.14.0-ocp-install-examples-rhel9.zip  lab-user@ssh.ocpv08.rhdp.net:/tmp/
----
+
NOTE: Change the port number, file name and path, ssh host name as per your lab environment.
...
cd ~/broker/operator
----
. Copy the AMQ Broker Operator installation and example files to this directory.
+
----
mv /tmp/amq-broker-operator-7.14.0-ocp-install-examples-rhel9.zip .
----
+
[NOTE]
The exact version number (e.g., `7.14.0`) may vary depending on the latest release. Always refer to the official Red Hat documentation for the most current download links and instructions. The following example uses `7.14.0`.
+
You would typically download this from the Red Hat Customer Portal. For demonstration purposes, assume you've downloaded `amq-broker-operator-7.14.0-ocp-install-examples-rhel9.zip` file.

.   Extract the contents of the downloaded archive.
+
[source,bash]
----
unzip amq-broker-operator-7.14.0-ocp-install-examples-rhel9.zip
----
+
