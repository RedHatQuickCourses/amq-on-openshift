...
. Create an OpenShift secret containing the broker's keystore. This secret will be referenced by the AMQ Broker Custom Resource (CR). The name of the resulting Keystore should be "broker.ks".
+
[source,bash]
----
...
. Create a broker trust store (`client.ts`) and import the client's public certificate into it. This trust store will be used by the broker to verify client identities.
+
[source,bash]
----
...
. Create an OpenShift secret containing the broker's trust store for client authentication. The name of the resulting TrustStore should be "client.ts".
+
[source,bash]
----
