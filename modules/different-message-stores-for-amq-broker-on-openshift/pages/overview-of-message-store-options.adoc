...
spec:
  deploymentPlan:
    ...
    storage:
      ...
      selector: {}
----
In the `storage` section of the `ActiveMQArtemis` Custom Resource (CR), you specify the desired size for the Persistent Volume. The AMQ Broker Operator then provisions the necessary PV/PVCs and mounts them to the broker pods at the default paths (`/opt/my-broker/data/journal`, etc.) within the container.
...
----
.  **Inspect Storage Directories**: Once inside the pod, navigate to the default data directory `/opt/my-broker/data` and observe the persistence files.
+
[source,bash]
----
ls -l /opt/my-broker/data/
ls -l /opt/my-broker/data/journal
ls -l /opt/my-broker/data/paging
ls -l /opt/my-broker/data/bindings
ls -l /opt/my-broker/data/largemessages
----
+
