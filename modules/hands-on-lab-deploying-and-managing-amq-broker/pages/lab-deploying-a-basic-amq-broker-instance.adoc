...
.From the OpenShift web console, Operators > Operator Hub > (Search for AMQ Broker) > Install "Red Hat Integration - AMQ Broker for RHEL 9 (Multiarch)" > Choose "a specific namespace (amq-broker-lab)"
...
```text
amq-broker-controller-manager-XXXXX-YYYYY   1/1     Running   0          3m
```

=== Step 4: Configure the SSL Secret

[source, bash]
----
keytool -genkeypair -alias broker -keyalg RSA -keysize 2048 -validity 365 -keystore broker.ks -storepass password -keypass password -dname "CN=*.my-openshift.domain, OU=IT, O=RedHat, L=Raleigh, ST=NC, C=US"

keytool -exportcert -alias broker -file broker.cer -keystore broker.ks -storepass password -noprompt

oc create secret generic my-ssl-secret \
...
This command creates a single-node AMQ Broker instance named `my-broker` in the `amq-broker-lab` namespace. It uses a specific Red Hat AMQ Broker image, enables anonymous access, and configures an acceptor for common messaging protocols on port 61617, which will be exposed externally outside of OpenShift. We also enable the Hawtio management console for potential later use.
...
[source,bash]
----
oc exec -it pod/my-broker-ss-0 -- bash
----
