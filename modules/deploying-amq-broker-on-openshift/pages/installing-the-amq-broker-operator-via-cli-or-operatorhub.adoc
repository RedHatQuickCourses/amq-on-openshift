...
*   The AMQ Broker Operator installation archive (`amq-broker-operator-7.13.3-ocp-install-examples-rhel8.zip` or similar) downloaded from the Red Hat Customer Portal. This archive contains the necessary deployment files, including `operator.yaml`.
...
. Locate the `AMQ Broker 7.13.3 Operator Installation and Example Files` and click `Download`. The `amq-broker-operator-7.13.3-ocp-install-examples-rhel8.zip` archive will download automatically.
...
[source,bash]
----
$ mv amq-broker-operator-7.13.3-ocp-install-examples-rhel8.zip ~/broker/operator
----
...
[source,bash]
----
$ unzip amq-broker-operator-operator-7.13.3-ocp-install-examples-rhel8.zip
----
+
...
[source,bash]
----
$ oc get deployment amq-broker-operator -n <project-name>
----
+
...
amq-broker-operator   1/1     1            1           2m
----
...
amq-broker-operator-xxxxxxxxxx-xxxxx  1/1     Running   0          2m
----
...
. You can also verify the Operator's pod running by navigating to *Workloads* > *Pods* in the left navigation and filtering by the installed namespace. Look for a pod named `amq-broker-operator-xxxxxxxxxx-xxxxx` with a `Running` status.
