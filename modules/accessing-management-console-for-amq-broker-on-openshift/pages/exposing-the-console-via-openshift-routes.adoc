...
```bash
oc get route -l ActiveMQArtemis=<your-broker-name>-console
```
...
Let's modify the route's hostname for easier access. We'll use the `ingressHost` attribute to append a custom host.

.Edit the `ActiveMQArtemis` CR again:
```bash
oc edit activemqartemises <your-broker-name>
```
.Modify the `spec.console` section to include `ingressHost`:
```yaml
# ... (existing content)
spec:
  # ... (other broker configurations)
  console:
    expose: true
    ingressHost: custom-host.io << Add this line
# ... (rest of the CR)
```
...
# ... (rest of the CR)
```
Choose the `ingressHost` as per your requirement.

.Verify the updated console route:
```bash
oc get route -l ActiveMQArtemis=<your-broker-name>-console
```
You should observe that the `HOST/PORT` has been updated to include your `custom-host.io` or `haw-console.mycompany.com` (depending on which attribute you used).
```
NAME                           HOST/PORT                                  PATH   SERVICES                   PORT    TERMINATION            WILDCARD
my-broker-console              my-broker-console-project.custom-host.io   /      my-broker-console          <all>   reencrypt/Redirect     None
```
...
spec:
  ...
  console:
    ...
    # ingressHost: custom-host.io << Can be left or removed
# ... (rest of the CR)
```
...
*   **"Application is not available"**: This often indicates an issue with the underlying service or broker pod. Ensure the broker pod is running and healthy:
    ```bash
    oc get pods -l ActiveMQArtemis=<your-broker-name>
