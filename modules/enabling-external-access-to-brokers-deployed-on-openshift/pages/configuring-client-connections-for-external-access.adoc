...
2.  Specify the path to the client's trust store: `trustStorePath=<path_to_truststore>`.
3.  Provide the trust store password: `trustStorePassword=<password>`.
...
trustStorePath=~/client.ts&trustStorePassword=myTrustStorePassword
----
...
[source,bash]
----
export BROKER_URL="tcp://YOUR_BROKER_ROUTE_HOSTNAME:61616?sslEnabled=true;trustStorePath=/path/to/truststore.jks;trustStorePassword=password"
mvn exec:java
----
