...
OpenShift's `StatefulSet` ensures that each pod replica has a sequential identity (e.g., `broker-name-ss-0`, `broker-name-ss-1`, etc.). This ordinal index is crucial for managing persistent volumes and ensuring proper failover and recovery mechanisms, particularly in clustered deployments.
...
[source,bash]
----
oc get pods -l ActiveMQArtemis=my-broker
oc get activemqartemis my-broker -o yaml | grep "size"
----

You should see one pod, e.g., `my-broker-ss-0`, and `replicas: 1`.
...
[source,bash]
----
oc get pods -l ActiveMQArtemis=my-broker --watch
----

You should eventually see two pods: `my-broker-ss-0` and `my-broker-ss-1`, both in `Running` and `Ready` states. Press `Ctrl+C` to exit the `watch` command.
...
The output should now show `size: 2`.
...
This command opens the CR in your default text editor. Navigate to `spec.deploymentPlan.size` and change its value from `2` back to `1`. Save and exit the editor.
...
spec:
  deploymentPlan:
      size: 1 # Change this from 2 to 1
    # ...
----
...
You should see `my-broker-ss-1` terminating, and eventually disappear, leaving only `my-broker-ss-0`. During this process, if there were any messages on `my-broker-ss-1`, the scaledown controller would have initiated a drainer pod to migrate them to `my-broker-ss-0`. While we can't easily observe the internal drainer pod for a simple scale-down without active messaging, it's important to understand that this mechanism is in place.
