...
[,bash]
----
oc login -u developer -p developer https://<openshift-cluster-url>:443
oc new-project amq-broker-project
----
. Deploy a simple application (e.g., an httpd deployment) to create some pods and a service.
+
[,bash]
----
...
my-httpd   ClusterIP   172.30.200.123   <none>        8080/TCP    2m
----
. Describe the `my-httpd` service to see its details, including its IP and selector.
+
[,bash]
----
...
----
+
You'll notice the `Selector` field, which links this Service to pods with the label `app=my-httpd`. The `Cluster-IP` is the stable internal IP.
...
Let's expose our `my-httpd` service using an OpenShift Route.

.Procedure
. Create a Route for the `my-httpd` service.
+
[,bash]
----
oc create route edge --service=my-httpd
----
+
...
my-httpd   my-httpd-amq-broker-project.apps.cluster-abc.example.com             my-httpd   8080   edge/Redirect      None
----
+
The `HOST/PORT` column shows the external URL for your service.
. Access the `my-httpd` application using the provided `HOST/PORT` in a web browser or with `curl`.
+
[,bash]
----
...
----
+
You should receive the Red Hat Enterprise Linux Test Page HTML.

.Customizing a Route Hostname
Now, let's see how `ingressHost` and `ingressDomain` relate to customizing routes. While we usually apply these in the `ActiveMQArtemis` CR for AMQ Broker, we can demonstrate the concept by editing our `my-httpd` route's hostname.

.Procedure
. Edit the `my-httpd` route.
+
[,bash]
----
oc edit route my-httpd
----
. Modify the `spec.host` field to a custom value, for example, `mycustommy-httpd.apps.cluster-abc.example.com`.
+
[,yaml]
----
...
metadata:
  name: my-httpd
spec:
  host: mycustomhttpd.apps.cluster-abc.example.com # <1>
  to:
    kind: Service
    name: my-httpd
  # ... other fields
----
<1> Replace `mycustomhttpd` and the domain with values appropriate for your cluster. Ensure the domain resolves to your OpenShift router.
. Save and exit the editor.
. Access the service using the new custom hostname:
+
[,bash]
----
curl http://mycustomhttpd.apps.cluster-abc.example.com
----
+
...
*   **Explicit Choice:** For AMQ Broker, the context indicates that "If your organization’s network policy require that you expose acceptors by using an ingress instead of a route," you explicitly add the `exposeMode` attribute and set its value to `ingress` in the `ActiveMQArtemis` CR. (Referenced from *CONTEXT*)
...
Let's modify our approach to use Ingress for the `my-httpd` service. (Note: For this to work seamlessly, your OpenShift cluster must have an Ingress Controller configured and running, which is typically the case by default.)

.Procedure
. First, remove the previously created `my-httpd` route.
+
[,bash]
----
oc delete route my-httpd
----
. Create an Ingress resource for the `my-httpd` service.
+
[,yaml]
.ingress.yaml
----
...
spec:
  rules:
  - host: my-httpd.apps.cluster-abc.example.com # <1>
    http:
      paths:
      - path: /
        pathType: Prefix
        backend:
          service:
            ...
            port:
              number: 80
----
<1> Replace `my-httpd` and the domain with values appropriate for your cluster. This hostname needs to be resolvable externally to your OpenShift Ingress controller.
...
[,bash]
----
oc get ingress my-httpd-ingress
----
+
You'll see output similar to:
+
...
my-httpd-ingress   <none>   my-httpd.apps.cluster-abc.example.com   <router-ip-or-hostname>   80      1m
----
+
The `HOSTS` column shows the external URL. The `ADDRESS` typically points to your OpenShift router's external IP or hostname.
. Access the `my-httpd` application using the provided `HOSTS` in a web browser or with `curl`.
+
[,bash]
----
curl http://my-httpd.apps.cluster-abc.example.com
----
+
You should again receive the Red Hat Enterprise Linux Test Page HTML. This demonstrates how Ingress can be used to expose services externally, similar to Routes.
