...
This activity demonstrates how to horizontally scale an AMQ Broker instance deployed on OpenShift using the `oc` command-line tool. This is a common operation for adjusting the capacity of your messaging infrastructure.
...
. *Verify Current Broker Replicas:*
   ...
   oc get pods -l ActiveMQArtemis=broker -n your-namespace
...
. *Scale Up the Broker Instance:*
   To horizontally scale out your broker, you'll increase the `replicas` count for the `ActiveMQArtemis` custom resource. The `oc scale` command is a convenient way to do this.

   For example, if you want to scale a broker named `broker-dr` in the `dr` namespace to 2 instances:

   
   
   oc -n dr scale ActiveMQArtemis broker-dr --replicas 2
   ...
   oc -n production scale ActiveMQArtemis broker-prod --replicas 2
...
. *Verify Scaled-Up Broker Instances:*
   ...
   oc get pods -l ActiveMQArtemis=broker-dr -n dr
   oc get pods -l ActiveMQArtemis=broker-prod -n production
...
. *Scale Down the Broker Instance:*
   ...
   oc -n dr scale ActiveMQArtemis broker-dr --replicas 1
   oc -n production scale ActiveMQArtemis broker-prod --replicas 1
...
This hands-on activity demonstrates the power and simplicity of horizontal scaling on OpenShift using the AMQ Broker Operator and `oc`. By adjusting the `replicas` count, you can dynamically adapt your messaging infrastructure to changing workload demands.