...
3.  **Add/Modify Configuration for Console Administration**:
    ...
    *   Configure the `console:` section with `expose: true`, 
    *   Configure the `adminUser: admin`, and `adminPassword: admin`. You can choose different credentials if desired.
...
4.  **Save and Apply the Changes**:
    ...
    [source,bash]
    ----
    oc get pod -l ActiveMQArtemis=my-console-broker -w
    ----
...
*   **Console Route not available**: Ensure `console.expose: true` is set in your CR and allow a few moments for the Operator to create the route. Check `oc get route -l ActiveMQArtemis=my-console-broker`.
