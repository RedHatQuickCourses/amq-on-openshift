= Persistence and Data Sharing for HA
...
+
Create a file named `broker-persistence.yaml` with the following `PersistentVolumeClaim` and the `ActiveMQArtemis` custom resource definition:
+
[source,yaml]
----
apiVersion: v1
...
spec:
    ...
    deploymentPlan:
        ...
        - name: extra-volume
          mountPath: /opt/amq-broker/data  
----
<1> Set `persistenceEnabled` to `false` to instruct the operator to not automatically provision and use persistent storage (as we are pre-provisioning it).
...
----
+
You should see a pod similar to `broker1-ss-0` enter the `Running` state and a PVC similar to `broker1-data-broker1-ss-0` in the `Bound` state.

. Send test messages to the broker.
+
You can use any client to send some messages to the broker. In this exercise we'll use the inbuild artemis cli.
+
[source,bash]
----
oc exec broker1-ss-0 -- /home/jboss/amq-broker/artemis producer --destination TEST --user admin --passwor admin
...
+
In the console, navigate to the `TEST` queue and confirm that it contains 10 messages.
...
[source,bash]
----
oc get pods -w -l app.kubernetes.io/name=broker1
----
+
Wait until the new `broker2-ss-0` pod is in the `Running` state.

. Verify message persistence after the restart.
+
Once the new broker pod is running, deploy a message consumer client to check if the previously sent messages are still available.
+
[source,bash]
----
oc exec broker1-ss-0 -- /home/jboss/amq-broker/artemis consumer --destination TEST --user admin --passwor admin
----
+
You should observe the 1000 messages being successfully consumed by the client, unequivocally confirming that they persisted through the broker pod restart.
+
You can also re-check the Hawtio console to confirm that `TEST` is now empty.
