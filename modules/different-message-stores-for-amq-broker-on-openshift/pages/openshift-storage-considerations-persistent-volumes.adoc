...
[TIP]
====
If you are planning to deploy a cluster of multiple brokers with persistent storage (e.g., a two-broker High Availability cluster), you must ensure that a corresponding number of Persistent Volumes are available – for instance, two PVs each providing 2 GiB of storage for a two-node cluster.
====
