...
spec:
  ...
  brokerProperties:
    ...
    # These paths are relative to the root of the mounted volume, which is /opt/my-broker/data by default
    - journalDirectory=/opt/my-broker/data/journal
    - pagingDirectory=/opt/my-broker/data/paging
    - bindingsDirectory=/opt/my-broker/data/bindings
    - largeMessagesDirectory=/opt/my-broker/data/large-messages
----

In this configuration:

*   `spec.deploymentPlan.persistenceEnabled: true` explicitly enables persistence for the broker instance.
*   `spec.deploymentPlan.storage.size` requests a Persistent Volume Claim (PVC) of 5Gi. The AMQ Broker Operator will automatically create this PVC if one with the specified size and access mode doesn't already exist and bind it to the broker pod. The Operator mounts this PVC at `/opt/my-broker/data` within the broker container.
*   The `brokerProperties` define the *paths* within the mounted volume (`/opt/my-broker/data`) where AMQ Broker will store its data. The Operator ensures these subdirectories are created and managed correctly.
...
.   **Verify the Deployment and PVC**
    ...
    c.  Inspect the mounted volume inside the pod to confirm the PVC is being used:
        [source,bash]
        ----
        oc exec -it $(oc get pod -l app=my-file-broker -o jsonpath='{.items[0].metadata.name}') -- df -h /opt/my-broker/data
        ----
        This command will show the mounted file system at `/opt/my-broker/data` and its usage, confirming the PV is correctly mounted within the broker pod.

    d.  List the created persistence directories within the mounted volume:
        [source,bash]
        ----
        oc exec -it $(oc get pod -l app=my-file-broker -o jsonpath='{.items[0].metadata.name}') -- ls -l /opt/my-broker/data
        ----
