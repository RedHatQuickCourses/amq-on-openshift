...
The AMQ Broker Operator expects secrets for TLS to contain two files: `broker.ks` and `client.ts`.

[IMPORTANT]
.AMQ Broker on OpenShift TLS Secret Naming Convention Nuance
When generating a secret for TLS, the AMQ Broker requires you to specify both a `Keystore` and a `Truststore` entry. The `Keystore` entry should be named broker.ks. For two-way TLS (mTLS) with the AMQ Broker, the file you provide for the `Truststore` entry should actually be your broker's trust store (`~/client.ts`), containing the public certificates of trusted clients.
...
oc create secret generic broker-cert-secret \
  ...
  --from-file=client.ts=~/broker.ts <1>
----
<1> Here, `broker.ks` maps to your `~/broker.ks` file. The `client.ts` key is given the content of `~/broker.ts`, and this file represents the *broker's trust store* for mTLS.
